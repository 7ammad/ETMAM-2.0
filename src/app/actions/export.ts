...
export async function exportTenderToExcel(tenderId: string): Promise<ExportExcelResult> {
  ...
  try {
    ...
    console.error("[exportTenderToExcel]", err);
    return { success: false, error: "حدث خطأ أثناء تصدير الملف" };
  }
}
...
export async function exportTendersToExcel(
  ...
  try {
    ...
    console.error("[exportTendersToExcel]", err);
    return { success: false, error: "حدث خطأ أثناء تصدير الملف" };
  }
}
...
export async function pushQualifiedTendersToOdoo(): Promise<PushAllQualifiedResult> {
  ...
  try {
    ...
    if (fetchError) return { success: false, error: "حدث خطأ أثناء جلب المنافسات المؤهلة" };
    ...
    console.error("[pushQualifiedTendersToOdoo]", err);
    return { success: false, error: "حدث خطأ أثناء الدفع إلى Odoo" };
  }
}
...
export async function pushTenderToOdoo(tenderId: string): Promise<PushToOdooResult> {
  ...
  try {
    ...
    if (result.lead_id != null) {
      await supabase
        ...
        .update({
          ...
          pushed_to_odoo_at: new Date().toISOString(),
    ...
    console.error("[pushTenderToOdoo]", err);
    return { success: false, error: "حدث خطأ أثناء الدفع إلى Odoo" };
  }
}
